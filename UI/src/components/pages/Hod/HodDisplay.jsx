...
export default function HodDisplay({setpending, setrejected, setsolved, setworking}) {
...
const navigate = useNavigate()
  ...
  useEffect(() => {
    const fetchComplain = async () => {
        ...
        await axios.get("/complain/hod/dept").then(res => {
          setComplains(res.data.complains);
          setFilter(res.data.complains);
          console.log(res.data.complains);
          setpending(complains.filter(c => c.status === "pending").length)
          setrejected(complains.filter(c => c.status === "rejected").length)
          setsolved(complains.filter(c => c.status === "solved").length)
          setworking(complains.filter(c => c.status === "working").length)
        })
  ...
  return (
    <div className='container'>
      ...
      <div className="search d-flex justify-content-center my-3">
        <input type="text" name="search" id="search" placeholder='search with complain ID' style={{ width: "25%", }} /><MDBBtn className="position-relative" style={{ marginLeft: "10px", marginRight: "30px" }}> Search</MDBBtn>
        <div className="visualization">
        <button class="btn btn-success" onClick={()=>navigate("/dpt/chart")}>visualization</button>
        </div>
